...
function NeighborhoodDiscovery(configuration) {
  ...
  function initializeMap() {
    ...
    widget.map.controls[google.maps.ControlPosition.TOP_LEFT].push(
      streetControlDiv
    );
    // 住所を逆ジオコーディングで取得する
      ...
      const processResult = function (place) {
        ...
        let index =
          spaceIndex === -1
            ...
            : commaIndex === -1
            ? spaceIndex
            : Math.min(spaceIndex, commaIndex);
        let trimmedAddress =
          index === -1 ? placeAddress : placeAddress.substr(0, index);
        // 検索機能やすでにピン立てしている場所をクリックした際にplaceIDを変える
...
function streetLocation(latlng, map) {
  ...
  service.getPanorama({ location: latlng, radius: 50 }, (data, status) => {
    ...
    if (status === google.maps.StreetViewStatus.OK) {
      ...
      document.querySelector('button[name="streetChange"]').textContent =
        "ストリートビュー";
